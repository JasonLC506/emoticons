...
if __name__ == "__main__":
    ...
    y_total = DTme.label2Rank(y_total)
    ...
    for size in sizes:
        ...
        start = datetime.now()
        result = crossValidate(x,y,"dT",cv=2, alpha=0.0, min_node=y.shape[1]*2)
        duration = datetime.now() -start
        print "total time: ", duration.total_seconds()
        with open("time_Mallows","a") as f:
            f.write("%d %f\n" % (size, duration.total_seconds()))
        with open("size_Mallows","a") as f:
            f.write("%d %f %f\n" %(size, result["size"][0], result["size"][1]))
        file = open("result_dt_mallows_nytimes_timesize.txt","a")
        file.write("# instances %d\n" % size)
